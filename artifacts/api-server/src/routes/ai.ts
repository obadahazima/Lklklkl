...
const getOverdueClientsDeclaration: FunctionDeclaration = {
  ...
  description:
    "اجلب قائمة الزبائن اللي *أنت* مدين لهم (دفعات/مصاريف معلّقة لسا ما دفعتها، رصيدهم يظهر بالأحمر بالتطبيق) من فترة، مرتّبة من الأكثر تأخراً. لا تشمل حالات إن الزبون هو المدين لك (معاملات معلّقة له عليك تحصيلها) — هاي عكس الاتجاه المطلوب هون. استخدمها عندما يسأل المستخدم عن الدفعات المتأخرة إلو لزبائنه أو المصاري اللي لسا ما دفعها، أو عندما تحلل البيانات وتلاحظ دفعات معلّقة قديمة وتريد تذكيره فيها.",
  parameters: {
    type: SchemaType.OBJECT,
    properties: {
      minDaysOverdue: {
        type: SchemaType.NUMBER,
        description: "أقل عدد أيام لاعتبار الدفعة متأخرة (افتراضياً 30 — شهر كامل)",
      },
    },
    required: [],
  },
...
const prepareWhatsappReminderDeclaration: FunctionDeclaration = {
  ...
  description:
    "جهّز رسالة تذكير دفع لزبون *مدين للبيزنس* (عليه مصاري لك) ورابط واتساب جاهز لإرسالها (wa.me). لا يرسل الرسالة تلقائياً — فقط يجهّز الرابط والنص ليضغط المستخدم عليه بنفسه. لا تستخدمها أبداً لزبون ظهر بنتيجة get_overdue_clients (هدول أنت المدين لهم، إرسال تذكير دفع لهم غلط بالاتجاه). استخدمها فقط بعد أن يوافق المستخدم صراحةً على إرسال تذكير لزبون محدد يدين له.",
  parameters: {
    type: SchemaType.OBJECT,
    properties: {
...
async function executeTool(
  ...
  if (name === "prepare_whatsapp_reminder") {
    ...
    for (const t of pending as TxRow[]) {
      // Only what the client actually owes (income/receipt) — a reminder message asking them to
      // pay shouldn't include pending payments going the other way (money the business owes them).
      if (t.type !== "income" && t.type !== "receipt") continue;
      amounts.set(t.currency, (amounts.get(t.currency) ?? 0) + Number(t.amount));
    }
    if (amounts.size === 0) return { error: "ما في مبلغ مستحق على هذا الزبون حالياً (كل معاملاته المعلّقة، إن وجدت، هي دفعات أنت مدين بها له، مش العكس)" };
    const amountsStr =
...
--- أدوات إضافية ---
- get_overdue_clients: تُرجع الزبائن اللي *أنت* مدين لهم (دفعات لسا ما دفعتها، رصيدهم أحمر بالتطبيق). استخدمها إذا سأل المستخدم عن دفعاته المتأخرة أو المصاري اللي لسا ما دفعها لزبائنه، أو إذا لاحظت من البيانات وجود دفعات معلّقة قديمة وتريد تنبيهه استباقياً في بداية المحادثة. **لا تقترح بعدها إرسال تذكير واتساب لهالزبائن** — أنت المدين، مو هم.
- prepare_whatsapp_reminder: عكس الاتجاه تماماً — لزبون *مدين لك* (عليه مصاري، رصيده أخضر بالتطبيق)، لتذكيره هو يدفعلك. لا تستدعِها إلا بعد موافقة صريحة من المستخدم على إرسال تذكير لزبون بعينه يدين له. النتيجة تحتوي رابط واتساب (whatsappLink) — اعرضه للمستخدم كرابط واضح ليضغط عليه، ولا تدّعِ أن الرسالة أُرسلت فعلياً؛ فقط الرابط جاهز، والمستخدم هو من يضغط إرسال.
