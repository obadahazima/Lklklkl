...
const getOverdueClientsDeclaration: FunctionDeclaration = {
  ...
  description:
    "اجلب قائمة الزبائن اللي *عليهم* مصاري لك (معاملات معلّقة من نوع دخل/قبض لم تُحصَّل بعد) من فترة، مرتّبة من الأكثر تأخراً. لا تشمل حالات إنك أنت مدين للزبون (دفعات معلّقة له) — هاي مش 'تأخر' من طرفه. استخدمها عندما يسأل المستخدم عن الزبائن المتأخرين بالدفع أو الذمم المستحقة له، أو عندما تحلل البيانات وتلاحظ تأخيراً وتريد اقتراح تذكير.",
  parameters: {
    type: SchemaType.OBJECT,
    properties: {
      minDaysOverdue: {
