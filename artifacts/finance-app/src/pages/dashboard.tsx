...
export default function Dashboard() {
  ...
  return (
    <div className="p-4 space-y-5 max-w-2xl mx-auto pb-24 lg:pb-6">
      ...
      {/* ── Overdue receivables alert (proactive, shown on open) ── */}
          ...
          <div className="flex items-start gap-3 p-3 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/50 cursor-pointer hover:bg-amber-100 dark:hover:bg-amber-950/50 transition-colors">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-amber-800 dark:text-amber-300">
                {language === "ar"
                  ? `${overdueClients.length} ${overdueClients.length === 1 ? "زبون مدين لك" : "زبائن مدينين لك"} من شهر أو أكثر`
                  : `${overdueClients.length} client${overdueClients.length === 1 ? "" : "s"} owe you money (a month or more)`}
              </p>
